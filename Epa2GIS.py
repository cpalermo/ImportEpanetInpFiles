...
from qgis.core import QgsFeature, QgsVectorLayer, QgsVectorFileWriter, QgsField, QgsPoint, QgsGeometry, \
    QgsMapLayerRegistry, QgsLayerTreeLayer, QgsProject, QgsCoordinateReferenceSystem
...
def epa2gis(inpname):
    ...
    for i in range(ss):
        if i < d.getBinNodeJunctionCount():
            try:
                ...
                featJ = QgsFeature()
                point = QgsPoint(float(x[ndIndexNew]), float(y[ndIndexNew]))
    ...
    posReact = QgsVectorLayer("point", "ReactionsI", "memory")
    ...
    if d.getBinLinkValveCount() > 0:
        ...
        for i, p in enumerate(d.getBinLinkValveIndex()):
            try:
                point1 = QgsPoint(float(x[ndCoordsID.index(d.getBinLinkFromNode()[p])]), float(y[ndCoordsID.index(d.getBinLinkFromNode()[p])]))
                point2 = QgsPoint(float(x[ndCoordsID.index(d.getBinLinkToNode()[p])]), float(y[ndCoordsID.index(d.getBinLinkToNode()[p])]))
            except:
                continue
            feature = QgsFeature()
    ...
    QgsMapLayerRegistry.instance().addMapLayer(ll, False)
