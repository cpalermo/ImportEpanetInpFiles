...
def epa2gis(inpname):
    ...
    if d.getBinLinkValveCount() > 0:
        ...
        for i, p in enumerate(d.getBinLinkValveIndex()):
            point1 = QgsPoint(float(x[ndID.index(d.getBinLinkFromNode()[p])]), float(y[ndID.index(d.getBinLinkFromNode()[p])]))
            point2 = QgsPoint(float(x[ndID.index(d.getBinLinkToNode()[p])]), float(y[ndID.index(d.getBinLinkToNode()[p])]))
            feature = QgsFeature()
